...
scp -i "$KEY_PATH" main.py requirements.txt $EC2_USER@$EC2_HOST:$DASHBOARD_DIR/
scp -i "$KEY_PATH" -r dashboard_pages/ $EC2_USER@$EC2_HOST:$DASHBOARD_DIR/
scp -i "$KEY_PATH" -r components/ $EC2_USER@$EC2_HOST:$DASHBOARD_DIR/
scp -i "$KEY_PATH" -r config/ $EC2_USER@$EC2_HOST:$DASHBOARD_DIR/
scp -i "$KEY_PATH" -r database/ $EC2_USER@$EC2_HOST:$DASHBOARD_DIR/
scp -i "$KEY_PATH" -r utils/ $EC2_USER@$EC2_HOST:$DASHBOARD_DIR/
